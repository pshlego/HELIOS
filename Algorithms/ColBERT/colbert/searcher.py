...
class Searcher:
    ...
    def encode(self, text: TextQueries, full_length_search=False):
        ...
        self.checkpoint.query_tokenizer.query_maxlen = self.config.query_maxlen #96 #512 #self.config.query_maxlen
        Q = self.checkpoint.queryFromText(queries, bsize=bsize, to_cpu=True, full_length_search=full_length_search)

        return Q

    def search(self, text: str, k=10, filter_fn=None, full_length_search=False, pids=None, pid_deleted_list=None):
        Q = self.encode(text, full_length_search=full_length_search)
        return self.dense_search(Q, k, filter_fn=filter_fn, pids=pids, pid_deleted_list=pid_deleted_list)
    ...
    def dense_search(self, Q: torch.Tensor, k=10, filter_fn=None, pids=None, pid_deleted_list=None):
        if k <= 10:
            ...
            if self.config.ncells is None:
                self.configure(ncells=4) #ncells=4
            if self.config.centroid_score_threshold is None:
                self.configure(centroid_score_threshold=0.4)
            if self.config.ndocs is None:
                self.configure(ndocs=max(k * 4, 4096))

        pids, scores = self.ranker.rank(self.config, Q, filter_fn=filter_fn, pids=pids, pid_deleted_list=pid_deleted_list)
