...
class IndexScorer(IndexLoader, CandidateGeneration):
    ...
    def retrieve(self, config, Q):
        Q = Q[:, :config.query_maxlen] #Q[:, :96] #Q[:, :config.query_maxlen]   # NOTE: Candidate generation uses only the query tokens
    ...
    def rank(self, config, Q, filter_fn=None, pids=None, pid_deleted_list=None):
        #config.query_maxlen = 96 #64
        with torch.inference_mode():
            ...
            if filter_fn is not None:
                filtered_pids = filter_fn(pids, pid_deleted_list)
