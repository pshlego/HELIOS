...
if __name__ == '__main__':
    ...
    print("5. Loading retrieved results...")
    retrieved_results = read_jsonl(results_path)
    print("5. Loaded " + str(len(retrieved_results)) + " retrieved results!")
    
    for top_k in [2, 5, 10, 20, 50]:
        recall_list = []
        for retrieved_result in tqdm(retrieved_results):
            ...
            recall, error_case  = evaluate(retrieved_graph, qa_data, table_key_to_content, passage_key_to_content, top_k)
            recall_list.append(recall)
                
        print(f"Recall@{top_k}: ", sum(recall_list)/len(recall_list))